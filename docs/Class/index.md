# Class

## Table of Contents

{{ BEGIN_TOC }}
- 控制类课程:
    - 电气控制技术[note]: Control/C_C01_Electrical_control/
    - 现代控制原理[note]: Control/C_C02_Modern_control/
    - 智能控制技术[note]: Control/C_C03_Intelligent_control/
    - 运动控制[note]: Control/C_C04_Motion_contorl/
    - 非线性规划[note]: Control/Optimize/C_C05_O01_Nonlinear_program/
    - 凸优化[note]: Control/Optimize/C_C05_O02_Convex_optimization/
- 电气与能源:
    - 智能供配电[note]: EE/C_E01_Power_distribution/
- 传感与检测:
    - 传感检测技术[note]: Sensor/C_S01_Sensing_detection/
    - PLC 课程内容[note]: Sensor/C_S02_PLC/
    - 传感检测复习资料[review]: class_source/C_S01/Review/
    - PLC 历年期末[exam]: class_source/C_S02/23Final/
- 机器学习课程:
    - 人工智能与机器学习[note]: MachineLearning/C_M01_AI_ML/
    - 机器学习实验一[lab]: class_source/C_M01/LAB1/
    - 机器学习实验二[lab]: class_source/C_M01/LAB2/
    - 机器学习实验三[lab]: class_source/C_M01/LAB3/
    - 机器学习实验四[lab]: class_source/C_M01/LAB4/
- 思政课程:
    - 毛概[note]: Ideological/C_I01_Mao_theories/
    - 发展对象[note]: Ideological/C_I02_PartydevExam/
    - 习概[note]: Ideological/C_I03_Xi_theories/
- 大数据与安全:
    - 信息安全导论[note]: Bigdata/C_B01_Information_safety/
    - 扫描工具[note]: Bigdata/C_B02_Scan/
    - 蜜罐部署[note]: Bigdata/C_B03_Honeypot/
    - 逆向工程[note]: Bigdata/C_B04_Reverse/
{{ END_TOC }}

<!-- - 控制类课程
    - 电气控制技术 <span class="toc-tag toc-tag-classnotes"></span>
    - 现代控制原理 <span class="toc-tag toc-tag-classnotes"></span>
    - 智能控制技术 <span class="toc-tag toc-tag-classnotes"></span>
    - 运动控制 <span class="toc-tag toc-tag-classnotes"></span>
- 传感与检测
    - 传感检测技术 <span class="toc-tag toc-tag-classnotes"></span>
    - PLC 课程内容 <span class="toc-tag toc-tag-classnotes"></span>
- 机器学习课程
    - 人工智能与机器学习 <span class="toc-tag toc-tag-classnotes"></span>
    - 系列实验 <span class="toc-tag toc-tag-reports"></span>
- 大数据与安全
    - 信息安全导论 <span class="toc-tag toc-tag-classnotes"></span>
    - 逆向工程 <span class="toc-tag toc-tag-classnotes"></span> -->
